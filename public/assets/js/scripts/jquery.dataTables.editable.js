  ...
  $.fn.makeEditable = function (options) {
    var iDisplayStart = 0,
      ...
      fnTakeRowDataFromFormElements = function (oForm) {
        ...
        $(
          "input:text[rel],[type=radio][rel][checked='checked'],input:hidden[rel],select[rel],textarea[rel],span.datafield[rel],input:checkbox[rel]",
          oForm
        ).each(function () {
          ...
          if (rel >= iColumnCount) {
            ...
            if (
              ...
              this.nodeName.toLowerCase() === "span" ||
            ...
            prop =
              oSettings.aoColumns[rel].mDataProp !== undefined
                ? oSettings.aoColumns[rel].mDataProp
                : oSettings.aoColumns[rel].mData;

            if (
              oSettings.aoColumns !== null &&
        ...
        prop =
          oSettings.aoColumns[0].mDataProp !== undefined
            ? oSettings.aoColumns[0].mDataProp
            : oSettings.aoColumns[0].mData;
      ...
      fnApplyEditable = function (aoNodes) {
          ...
          oDefaultEditableSettings = {
            ...
            onsubmit: function (settings, original) {
              ...
              if (
                ...
                if (input.length === 1) {
                  ...
                  if (
                    oEditElement.nodeName.toLowerCase() === "select" ||
                    oEditElement.tagName.toLowerCase() === "select"
                  ) {
                    sNewCellDisplayValue = $(
                      "option:selected",
                      oEditElement
                    ).text(); //For select list use selected text instead of value for displaying in table
                  } else {
                ...
                if (
                  !properties.fnOnEditing(
                    input,
                    settings,
                    original.revert,
                    fnGetCellID(original)
                  )
                ) {
                  return false;
                }
                ...
                if (
                  ...
                  settings.oValidationOptions === null
                ) {
                  return true;
                }
            ...
            submitdata: function (value, settings) {
              ...
              if (
                ...
                properties.aoColumns[columnId] === null
              ) {
            ...
            callback: function (sValue, settings) {
              ...
              if (sValue.indexOf(properties.sFailureResponsePrefix) > -1) {
                ...
                properties.fnShowError(
                  sValue.replace(properties.sFailureResponsePrefix, "").trim(),
                  "update"
                );
                status = "failure";
              } else {
                if (
                  properties.sSuccessResponse === "IGNORE" ||
                  (properties.aoColumns !== null &&
                    properties.aoColumns[aPos[2]] !== null &&
                    properties.aoColumns[aPos[2]].sSuccessResponse ===
                      "IGNORE") ||
                  sNewCellValue === null ||
        ...
        if (properties.aoColumns !== null) {
          for (
            iDTindex, iDTEindex;
            iDTindex < oSettings.aoColumns.length;
            iDTindex += 1
          ) {
            if (oSettings.aoColumns[iDTindex].bVisible) {
              //if DataTables column is visible
              ...
              oColumnSettings = $.extend(
                ...
                properties.aoColumns[iDTEindex]
              );
              iDTEindex += 1;
          ...
          cells.editable(
            properties.sUpdateURL,
            $.extend({}, oDefaultEditableSettings, properties.oEditableSettings)
          );
        }
      },
      /**
      ...
      fnOnRowAdded = function (data) {
        var values, rtn, oTRAdded, prop, keys;
              ...
              $(oAddNewRowForm).ajaxSubmit({
                ...
                success: function (response, statusString, xhr) {
                  if (xhr.responseText.toLowerCase().indexOf("error") !== -1) {
                    properties.fnEndProcessingMode();
                    properties.fnShowError(
                      xhr.responseText.replace("Error", ""),
                      "add"
                    );
      ...
      fnOnRowDelete = function (event) {
        ...
        if (!properties.bUseKeyTable) {
          if (
            $("tr." + properties.sSelectedRowClass + " td", oTable).length === 0
          ) {
            //oDeleteRowButton.attr("disabled", "true");
          ...
          nSelectedCell = $(
            "tr." + properties.sSelectedRowClass + " td",
            oTable
          )[0];
        } else {
      ...
      fnGetActionSettings = function (sAction) {
        if (properties.aoTableAction) {
          properties.fnShowError(
            "Configuration error - aoTableAction setting are not set",
            sAction
          );
        }
        var i = 0;
        ...
        properties.fnShowError(
          "Cannot find action configuration settings",
          sAction
        );
      },
      /**
            ...
            if (
              ...
              if (this.multiple === true) {
                ...
                for (var i = 0; i <= this.options.length - 1; i += 1) {
                  if (
                    jQuery.inArray(
                      this.options[i].text.toLowerCase().trim(),
                      aoCellValues
                    ) !== -1
                  ) {
                ...
                for (i = 0; i <= this.options.length - 1; i++) {
                  if (
                    this.options[i].text.toLowerCase() ===
                    sCellValue.toLowerCase()
                  ) {
        ...
        if (properties.fnOnBeforeAction(sAction)) {
          if (jActionForm.valid()) {
            ...
            if (properties.bUseFormsPlugin) {
              ...
              oAjaxSubmitOptions = $.extend(
                {},
                properties.oAjaxSubmitOptions,
                oAjaxSubmitOptions
              );
    ...
    return this.each(function () {
      ...
      if (oAddNewRowForm.length !== 0) {
        ...
        for (i; i < iColumnCount; i += 1) {
          if ($("[rel=" + i + "]", oAddNewRowForm).length === 0) {
            properties.fnShowError(
              "In the form that is used for adding new records cannot be found an input element with rel=" +
                i +
                " that will be bound to the value in the column " +
                i +
        ...
        if (oAddNewRowButton.length !== 0) {
          ...
          if ($(properties.sAddDeleteToolbarSelector).length === 0) {
            throw (
              "Cannot find a button with an id '" +
        ...
        if (oConfirmRowAddingButton.length === 0) {
          ...
          if (
            properties.oAddNewRowOkButtonOptions.text === null ||
          ...
          properties.oAddNewRowOkButtonOptions.id =
            properties.sAddNewRowOkButtonId;
        ...
        if (oCancelRowAddingButton.length === 0) {
          ...
          if (
            properties.oAddNewRowCancelButtonOptions.text === null ||
          ...
          properties.oAddNewRowCancelButtonOptions.id =
      ...
      if (oCancelRowAddingButton !== null) {
        if (properties.oAddNewRowCancelButtonOptions !== null) {
          oCancelRowAddingButton.button(
            properties.oAddNewRowCancelButtonOptions
          );
        }
      }
      ...
      if (!properties.bUseKeyTable) {
        ...
        $("tbody", oTable).click(function (event) {
          if (
            $(event.target.parentNode).hasClass(properties.sSelectedRowClass)
          ) {
      ...
      if (properties.aoTableActions !== null) {
        for (var i = 0; i < properties.aoTableActions.length; i++) {
          var oTableAction = $.extend(
            { sType: "edit" },
            properties.aoTableActions[i]
          );
          ...
          if (oActionForm.length !== 0) {
            ...
            if (oActionFormLink.length !== 0) {
              oActionFormLink.live("click", function () {
                ...
                if (sActionFormId === "") {
                  properties.fnShowError(
                    "Cannot find a form with an id " +
                      sActionFormId +
                      " that should be associated to the action - " +
                      sAction,
                    sAction
                  );
                }
    ...
    function fnUpdateRowOnSuccess(nActionForm) {
      ...
      for (var rel = 0; rel < iColumnCount; rel++) {
        ...
        if (sCellValue !== undefined) oTable.fnUpdate(sCellValue, iRowID, rel);
      }
